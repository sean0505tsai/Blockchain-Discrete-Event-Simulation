...
本模擬器原本為 CS5068701計算機模擬 課程之期末專案開發使用。為了契合本區塊鏈課程的評分精神，我將針對該模擬器產出的數據，重新撰寫一份專注於「區塊鏈物理機制探討（如長尾效應、空區塊現象）」的分析報告，並修改該模擬器使其更為接近真實比特幣系統。  
...
交易生成機制採用 Poisson 過程，交易大小則以三個常態分布的混合模型來模擬，分別為：
...
- 為求模擬的簡化與效率，當前區塊的打包過程中不會考慮交易費用（fee）對交易優先級的影響，完全以交易大小為基礎來決定是否能被打包進當前區塊。
...
| num_blocks_to_simulate | 144 | 統計區塊數 N (不含暖機) |
