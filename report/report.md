...
本模擬器原本為 *CS5068701計算機模擬* 課程之期末專案開發使用。為了契合本區塊鏈課程的評分精神，報告中將針對該模擬器產出的數據，重新撰寫一份針對「區塊鏈物理機制探討（如網路壅塞、空區塊現象）」的分析報告，並進一步修改該模擬器使其更為接近真實比特幣系統。  
...
比特幣的交易生成是由多種不同協議腳本與使用者行為交織而成的客觀事實。單一統計分佈無法捕捉這種去中心化生態的特徵。因此，本模擬器進一步改良原本的靜態容量限制(不考慮交易大小，且固定每區塊容量上限3000筆交易)，改採用高斯混合模型（GMM），搭配Box-Muller 轉換法來模擬交易大小的分佈。交易大小則以三個常態分布的混合模型來模擬，分別為：
